...
class individual():
    ...
    def update_energy(self, grid: np.ndarray) -> None:
        ...
        """
            ...
            Each step costs energy (-1).
            Finding food on the grid can restore energy, whereas finding poison decreases it.
        ...
        # Make step to current position
        self.energy -= 1

        # Check the grid for foof or pison
        if grid[self.x, self.y] == 1:
            grid[self.x, self.y] = 0    # Eat food
            self.energy += 2            # Restore energy
        elif grid[self.x, self.y] == -1:
            grid[self.x, self.y] = 0    # Eat poison
    ...
    def move(self, grid: np.ndarray) -> None:
        ...
        """
            ...
            N.B. If possible directions are different from genes, than choose random (again).
        ...
        if self.energy > 0:
            ...
            # Count different possible directions
            genes_set = set(neighbours)

            # Add compatible genes only
            appo_genes = [gene for gene in self.genes if gene in genes_set]


            if len(genes_set) == 1 or len(appo_genes) == 0:
                self.step(random.randint(0,3))
            else:
                # Make weights for rigged wheel
                count_genes = Counter(appo_genes)
                list_genes, weights = [], []
...
def crossover(parent1: list, parent2: list) -> list:
    ...
    '''
        ...
        It creates two child offspring of n genes selected from both of the parents with 
...
def generate_population(n_pop: int, energy: int, n_genes: int, D: int) -> list:
    ...
    for _ in range(n_pop):
        ...
        genes = random.choices([-1,0,1], k=n_genes)
